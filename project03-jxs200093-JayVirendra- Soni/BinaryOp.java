  ...
  public VarType typeCheck() throws Exception{
    if (operator.equals("+") || operator.equals("-") || operator.equals("*") || operator.equals("/")){
        ...
        if (! lhsType.equals(VarType.Float) && !lhs.equals(VarType.Int)){
            System.out.println("Error: Cant apply arithmetic operator " + operator + " on expression of type: " + lhsType + " and " + rhsType);
            System.exit(0);
        }
        if (! rhsType.equals(VarType.Float) && !rhsType.equals(VarType.Int)){
            System.out.println("Error: Cant apply arithmetic operator " + operator + " on expression of type: " + rhsType  + " and " + rhsType);
            System.exit(0);
        }
        ...
        if (!lhsType.equals(VarType.Bool) && !lhsType.equals(VarType.Int)){
            System.out.println("Error: Incompatible types: " + lhsType + " cannot be converted to Boolean");
            System.exit(0);
        }
        if (!rhsType.equals(VarType.Bool) && !rhsType.equals(VarType.Int)){
            System.out.println("Error: Incompatible types: " + rhsType + " cannot be converted to Boolean");
            System.exit(0);
        }
        ...
        if (!lhsType.equals(VarType.Int) && !lhsType.equals(VarType.Float)){
            System.out.println("Error: Incompatible types: " + operator + " and " + lhsType);
            System.exit(0);
        }

        if (!rhsType.equals(VarType.Int) && !rhsType.equals(VarType.Float)){
            System.out.println("Error: Incompatible types: " + operator + " and " + rhsType);
            System.exit(0);
        }
        return VarType.Bool;
    }
