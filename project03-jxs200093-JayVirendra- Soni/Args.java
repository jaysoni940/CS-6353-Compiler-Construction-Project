...
public class Args extends SuperToken implements Token{
    ...
    public void typeCheck(String fnName, List<VarType> methodArgs) throws Exception{
        ...
        if (argTypes.size() != methodArgs.size()){
            System.out.println("Error: Function " + fnName + " expected " + methodArgs.size() + " arguments, " + argTypes.size() + " provided");
            System.exit(0);
        }
        ...
        for (int i=0; i < methodArgs.size(); i++){
            ...
            if (!isCoercible(expectedArg, actualArg)){
                System.out.println("Error: Expected argument of type " + expectedArg + " in position " + i + " of function " + fnName + ", " + actualArg + " provided");
                System.exit(0);
            }
        }
