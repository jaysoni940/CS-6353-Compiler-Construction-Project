class Readlist extends SuperToken implements Token{
    ...
    public VarType typeCheck() throws Exception {
        ...
        if (nameStatus == null){
            System.out.println(" Error: Variable " + name.ID + " not found in the scope");
            System.exit(0);
        }

        if (nameStatus.isFinal){
            System.out.println(" Error: Cant use read with final variable");
            System.exit(0);
        } else if (nameStatus.isMethod){
            System.out.println(" Error: Cant use read with method");
            System.exit(0);
        } else if (nameStatus.isArray && name.expr == null){
            System.out.println(" Error: Cant use read with array");
            System.exit(0);
        }
